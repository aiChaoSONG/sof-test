...
def do_wave_compare(mode, wave_path, ref_path):
    fs_wav, wave = wavefile.read(wave_path.strip())
    fs_ref, ref_wave = wavefile.read(ref_path.strip())
    if fs_wav != fs_ref:
        print('Can not compare wave with different sample rate')
        sys.exit(1)
    if mode == 'smart_amp':
        compare_wav_smart_amp(wave, ref_wave, fs_wav)

# remove digital zeros in two sides
# amplitude below 100 are regarded as zero, this is true for wave go through DAC-ADC
def trim_wave(wave):
    wave_mono = wave[:,0]
    left_idx = 0
...
def compare_wav_smart_amp(wave, ref_wave, fs):
    ...
    if compare_result0_1 and compare_result2_3 and smart_amp_delay < 5.:
...
def parse_cmdline():
    ...
    parser.add_argument('-c', '--compare', type=str, choices=['smart_amp'], help='compare recorded wave and reference wave')
...
def main():
    ...
    if cmd_args['compare'] is not None:
        do_wave_compare(cmd_args['compare'], cmd_args['recorded_wave'], cmd_args['reference_wave'])
